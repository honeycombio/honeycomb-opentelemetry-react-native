...
### Stuck event loop
The Honeycomb React Native SDK comes with a stuck event loop detection instrumentation. 

#### Configuration 
| Option                        | Type                                | Required? | default value | Description                                                                                                                                                                         |
|-------------------------------|-------------------------------------|-----------|---------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `loopSampleIntervalMs`.       | number                              | No        | 200           | Duration (in milliseconds) between each sampling of the event loop duration.                                                                                                        |
...
| `applyCustomAttributesOnSpan` | ApplyCustomAttributesOnSpanFunction | No.       | undefined     | A callback function for adding custom attributes to the span when a stuck event loop is recorded. Will automatically be applied to all spans generated by the auto-instrumentation. |


#### Fields
When a stuck event loop is detected, it will emit a 'slow event loop' span with the following fields.
